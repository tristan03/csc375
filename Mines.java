...
class Game extends JPanel {
    ...
    void play(Game mines) {
            ...
            for (int col = 0; col < cols; col++) {
                ...
                button.addMouseListener(new MouseAdapter() {
                    ...
                    public void mouseClicked(MouseEvent e) {
                        ...
                        lastMove = buttonGrid[clickedRow][clickedCol];
        ...
        JMenuItem gameTypeCustom = new JMenuItem("Custom");
        ...
        // for changing game types (size & mine amounts)
        // easy mode
        ...
        // custom mode
        // TODO: FIX
        gameTypeCustom.addActionListener(e -> {
            int customNumRows = 0;
            int customNumCols = 0;
            int customNumMines = 0;
            ...
            int result = JOptionPane.showConfirmDialog(frame, panel, "Custom Game Settings",
                    JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);

            if (result == JOptionPane.OK_OPTION) {
                customNumRows = Integer.parseInt(rowsField.getText());
                customNumCols = Integer.parseInt(colsField.getText());
                customNumMines = Integer.parseInt(minesField.getText());
            }

            int cellWidth = 30;
            int cellHeight = 30;
            int extraWidth = 100;  // extra width for padding, borders, etc.
            int extraHeight = 100; // extra height for menus, status bars, etc.

            int totalWidth = cols * cellWidth + extraWidth;
            int totalHeight = rows * cellHeight + extraHeight;

            int minWidth = 300; // Minimum width
            int minHeight = 300; // Minimum height
...
//            totalHeight = Math.max(totalHeight, minHeight);
            ...
            Game customGame = new Game(totalHeight, totalWidth, customNumRows, customNumCols, customNumMines);

            refreshFrame(frame, customGame);
        });

        gameTypeMenu.add(gameTypeEasy);
        gameTypeMenu.add(gameTypeMedium);
        gameTypeMenu.add(gameTypeHard);
        gameTypeMenu.add(gameTypeCustom);
        ...
        undoMoveItem.addActionListener(e -> {
            for (int i = 0; i < buttonGrid.length; i++) {
                for (int j = 0; j < buttonGrid[i].length; j++) {
                    if (lastMove.equals(buttonGrid[i][j])) {
                        lastMove.setBorder(new BevelBorder(BevelBorder.RAISED));
                        lastMove.setBackground(Color.lightGray);
                        lastMove.setText("");
                    }
                }
            }
    ...
    private void updateTimer() {
        ...
        if (secondCounter < 10) {
            timerLabel.setText(" [" + minuteCounter + ":0" + secondCounter + "]  ");
